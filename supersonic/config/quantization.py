...
@dataclass
class SuperSonicConfig(QuantizationConfigMixin):
    """
    This is a wrapper class about all possible attributes and features that you can play with a model that has been
    loaded using `supersonic`.

    This replaces `load_in_8bit` or `load_in_4bit` therefore both options are mutually exclusive.

    Currently supports `LLM.int8()`, `FP4`, `NF4`, `AWQ`, and `AQLM` quantization methods.

    Args:
        load_in_8bit (`bool`, *optional*, defaults to `False`):
            This flag is used to enable 8-bit quantization with LLM.int8().
        load_in_4bit (`bool`, *optional*, defaults to `False`):
            This flag is used to enable 4-bit quantization by replacing the Linear layers with FP4/NF4 layers
        llm_int8_threshold (`float`, *optional*, defaults to 6.0):
            This corresponds to the outlier threshold for outlier detection as described in `LLM.int8() : 8-bit Matrix
            Multiplication for Transformers at Scale` paper: https://huggingface.co/papers/2208.07339 Any hidden states value
            that is above this threshold will be considered an outlier and the operation on those values will be done
        ...
        llm_int8_has_fp16_weight (`bool`, *optional*, defaults to `False`):
            This flag runs LLM.int8() with 16-bit main weights. This is useful for fine-tuning as the weights do not
            have to be converted back and forth for the backward pass.
        supersonic_4bit_compute_dtype (`tinygrad.dtype` or str, *optional*, defaults to `dtypes.float32`):
            This sets the computational type which might be different than the input type. For example, inputs might be
            fp32, but computation can be set to bf16 for speedups.
        supersonic_4bit_quant_type (`str`,  *optional*, defaults to `"fp4"`):
            This sets the quantization data type. Options are FP4, NF4, AWQ, and AQLM data types
            which are specified by `fp4`, `nf4`, `awq`, or `aqlm`.
        supersonic_4bit_use_double_quant (`bool`, *optional*, defaults to `False`):
            This flag is used for nested quantization where the quantization constants from the first quantization are
            quantized again.
        supersonic_4bit_quant_storage (`tinygrad.dtype` or str, *optional*, defaults to `dtypes.uint8`):
    ...
    def __init__(
        ...
        llm_int8_enable_fp32_cpu_offload=False,
        llm_int8_has_fp16_weight=False,
        supersonic_4bit_compute_dtype=None,
        supersonic_4bit_quant_type="nf4",
        supersonic_4bit_use_double_quant=False, #not yet implemented
        supersonic_4bit_quant_storage=None,
        **kwargs,
    ):
        self.quant_method = QuantizationMethod.SUPERSONIC
        ...
        self.llm_int8_enable_fp32_cpu_offload = llm_int8_enable_fp32_cpu_offload
        self.llm_int8_has_fp16_weight = llm_int8_has_fp16_weight
        self.supersonic_4bit_quant_type = supersonic_4bit_quant_type
        self.supersonic_4bit_use_double_quant = supersonic_4bit_use_double_quant

        if supersonic_4bit_compute_dtype is None:
            self.supersonic_4bit_compute_dtype = dtypes.float32
        elif isinstance(supersonic_4bit_compute_dtype, str):
            dtype_map = {
                "float16": dtypes.float16,
                "float32": dtypes.float32,
                "bfloat16": dtypes.bfloat16,
            }
            if supersonic_4bit_compute_dtype in dtype_map:
                self.supersonic_4bit_compute_dtype = dtype_map[supersonic_4bit_compute_dtype]
            else:
                raise ValueError(f"Unsupported dtype string: {supersonic_4bit_compute_dtype}")

        elif isinstance(supersonic_4bit_compute_dtype, DType):
            self.supersonic_4bit_compute_dtype = supersonic_4bit_compute_dtype
        else:
            raise ValueError(
                f"supersonic_4bit_compute_dtype must be a string or a tinygrad.dtype.DType, "
                f"but got {type(supersonic_4bit_compute_dtype)}"
            )

        if supersonic_4bit_quant_storage is None:
            self.supersonic_4bit_quant_storage = dtypes.uint8
        elif isinstance(supersonic_4bit_quant_storage, str):
            storage_dtype_map = {
                "float16": dtypes.float16,
                "float32": dtypes.float32,
                "int8": dtypes.int8,
            ...
            if supersonic_4bit_quant_storage not in storage_dtype_map:
                raise ValueError(
                    f"`supersonic_4bit_quant_storage` must be a valid string (one of {list(storage_dtype_map.keys())}) "
                )
            self.supersonic_4bit_quant_storage = storage_dtype_map[supersonic_4bit_quant_storage]
        elif isinstance(supersonic_4bit_quant_storage, DType):
            self.supersonic_4bit_quant_storage = supersonic_4bit_quant_storage
        else:
            raise ValueError(
                f"supersonic_4bit_quant_storage must be a string or a tinygrad.dtype.DType, "
                f"but got {type(supersonic_4bit_quant_storage)}"
            )

        if kwargs:
            print(f"Unused kwargs: {list(kwargs.keys())}. These kwargs are not used in {self.__class__}.")
    ...
    def post_init(self):
        ...
        if self.supersonic_4bit_compute_dtype is not None and not isinstance(self.supersonic_4bit_compute_dtype, DType):
            raise TypeError("supersonic_4bit_compute_dtype must be a tinygrad DType")

        if not isinstance(self.supersonic_4bit_quant_type, str):
            raise TypeError("supersonic_4bit_quant_type must be a string")

        valid_quant_types = ["fp4", "nf4", "awq", "aqlm", "int4", "af4"]
        if self.supersonic_4bit_quant_type not in valid_quant_types:
            raise ValueError(f"supersonic_4bit_quant_type must be one of {valid_quant_types}")

        if not isinstance(self.supersonic_4bit_use_double_quant, bool):
            raise TypeError("supersonic_4bit_use_double_quant must be a boolean")


        if self.load_in_4bit:
            # dd version check later if needed
            # if self.load_in_4bit and not version.parse(importlib.metadata.version("bitsandbytes")) >= version.parse(
            #     "0.39.0"
            # ):
             # raise ValueError(
    ...
    def quantization_method(self):
        ...
        if self.load_in_8bit:
            return "llm_int8"
        elif self.load_in_4bit and self.supersonic_4bit_quant_type == "fp4":
            return "fp4"
        elif self.load_in_4bit and self.supersonic_4bit_quant_type == "nf4":
            return "nf4"
        elif self.load_in_4bit and self.supersonic_4bit_quant_type == "awq":
            return "awq"
        elif self.load_in_4bit and self.supersonic_4bit_quant_type == "aqlm":
            return "aqlm"
        else:
            return None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if hasattr(output["supersonic_4bit_compute_dtype"], "name"):
            output["supersonic_4bit_compute_dtype"] = output["supersonic_4bit_compute_dtype"].name
        else:
            output["supersonic_4bit_compute_dtype"] = str(output["supersonic_4bit_compute_dtype"])

        if hasattr(output["supersonic_4bit_quant_storage"], "name"):
            output["supersonic_4bit_quant_storage"] = output["supersonic_4bit_quant_storage"].name
        else:
            output["supersonic_4bit_quant_storage"] = str(output["supersonic_4bit_quant_storage"])
    ...
    def to_diff_dict(self) -> dict[str, Any]:
        ...
        default_config_dict = SuperSonicConfig().to_dict()
